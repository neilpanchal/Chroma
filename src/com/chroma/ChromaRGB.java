...
public class ChromaRGB extends ChromaColor {
	...
	public ChromaLAB getChromaLAB() {
		...
		double x = ChromaUtil
				.xyz_lab((0.4124564 * rgb_R_ + 0.3575761 * rgb_G_ + 0.1804375 * rgb_B_)
						/ CONSTANT_X);
		double y = ChromaUtil
				.xyz_lab((0.2126729 * rgb_R_ + 0.7151522 * rgb_G_ + 0.0721750 * rgb_B_)
						/ CONSTANT_Y);
		double z = ChromaUtil
				.xyz_lab((0.0193339 * rgb_R_ + 0.1191920 * rgb_G_ + 0.9503041 * rgb_B_)
						/ CONSTANT_Z);

		return new ChromaLAB(116.0 * y - 16.0, 500.0 * (x - y),
				200.0 * (y - z), alpha);
	}

	@Override
	public ChromaLCH getChromaLCH() {
		return getChromaLAB().getChromaLCH();
	}
